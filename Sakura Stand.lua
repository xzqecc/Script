...
function menu.Initialize(menutable)
	...
	do
		...
		getgenv().menuname = "xzs Sakura Stand | https://discord.gg/7sUzjKvjNY"
						...
						if pass then
							for k2, v2 in pairs(v1) do
								...
								elseif v2[2] == DROPBOX then
									...
									elseif
										menu:MouseInMenu(v2[3][1], v2[3][2], v2[3][3], 24 * (#v2[6] + 1) + 3)
										and v2[5]
									then
										for i = 1, #v2[6] do
											if
								...
								elseif v2[2] == COMBOBOX then
									...
									elseif
										menu:MouseInMenu(v2[3][1], v2[3][2], v2[3][3], 24 * (#v2[1] + 1) + 3)
										and v2[5]
									then
										for i = 1, #v2[1] do
											if
			...
			if menu.textboxopen then
				...
				if inputObject.KeyCode == Enum.KeyCode.A then
					if
						...
						and INPUT_SERVICE:IsKeyDown(Enum.KeyCode.LeftControl)
					then
						menu.selectall = true
						local textbox = menu.textboxopen
...
if menu.game == "uni" then --SECTION UNIVERSAL
	...
	local wepesp = allespf[7]

	local wepespnum = #wepesp

	local nade_esp = allespf[8]

	local nade_espnum = #nade_esp
	for i = 1, Players.MaxPlayers do
		Draw:Circle(false, 0, 0, 0, 1, 32, { 0, 0, 0, 0 }, allespf[11][1])
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 255, 255, 255, 255 }, { 0, 0, 0 }, allespf[11][2])
	end
	for i = 1, 50 do
		Draw:FilledRect(false, 20, 20, 2, 20, { 30, 30, 30, 255 }, allespf[10][5])
	end
	for i = 1, 50 do
		Draw:FilledRect(false, 20, 20, 2, 20, { 30, 30, 30, 255 }, allespf[10][1])
	end

	local this_is_really_ugly = 50 - 15 * 1.7
	for i = 1, 50 do
		Draw:FilledRect(false, 20, 20, 2, 20, {this_is_really_ugly, this_is_really_ugly, this_is_really_ugly, 255}, allespf[10][3])
	end
	for i = 1, 19 do
		Draw:FilledRect(false, 0, 0, 10, 1, { 50 - i * 1.7, 50 - i * 1.7, 50 - i * 1.7, 255 }, allespf[10][2])
	end
	for i = 1, 50 do
		...
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 255, 255, 255, 255 }, { 0, 0, 0 }, allespf[10][4])
	end
	for i = 1, 4 do
		allespf[9][i] = {}
		for k = 1, 2 do
			Draw:Circle(false, 0, 0, 0, 1, 32, { 0, 0, 0, 0 }, allespf[9][i])
		end
	end
	for i = 1, 20 do
	...
	for i = 1, 35 do
		for i_ = 1, 2 do
			Draw:Triangle(false, i_ == 1, nil, nil, nil, { 255 }, allespf[5][i_])
		end

		local skel = allespf[1]
		local box = allespf[2]
		local hp = allespf[3]
		local text = allespf[4]
		local icons = allespf[3][7]
		local arrows = allespf[5]
	...
	for i = 1, Players.MaxPlayers do
		...
		-- Draw:OutlinedText(text, font, visible, pos_x, pos_y, size, centered, clr, clr2, tablename)
		...
		Draw:OutlinedText("", 1, false, 20, 20, 13, true, { 255, 255, 255, 255 }, { 0, 0, 0 }, allesp.team)
	end

		for i = 1, 400 do
			Draw:FilledRect(false, 20, 20, 20, 20, { 0, 0, 0, 220 }, allesp.mobfilledbox)

			Draw:OutlinedRect(false, 20, 20, 20, 20, { 0, 0, 0, 220 }, allesp.mobinnerbox)
			Draw:OutlinedRect(false, 20, 20, 20, 20, { 0, 0, 0, 220 }, allesp.mobouterbox)
			Draw:OutlinedRect(false, 20, 20, 20, 20, { 255, 255, 255, 255 }, allesp.mobbox)

			Draw:FilledRect(false, 20, 20, 4, 20, { 10, 10, 10, 215 }, allesp.mobhealthouter)
			Draw:FilledRect(false, 20, 20, 20, 20, { 255, 255, 255, 255 }, allesp.mobhealthinner)

			Draw:OutlinedText("", 1, false, 20, 20, 13, false, { 255, 255, 255, 255 }, { 0, 0, 0 }, allesp.mobhptext)
			Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 255, 255, 255, 255 }, { 0, 0, 0 }, allesp.mobdistance)
			Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 255, 255, 255, 255 }, { 0, 0, 0 }, allesp.mobname)
		end
	...
	for i = 1, 200 do
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 255, 255, 255, 255 }, { 0, 0, 0 }, allesp.Box)
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 255, 255, 255, 255 }, { 0, 0, 0 }, allesp.bag)
		Draw:OutlinedText("", 2, false, 20, 20, 13, true, { 255, 255, 255, 255 }, { 0, 0, 0 }, allesp.artifact)
	end
...
menu.connections.renderstepped2 = game.RunService.RenderStepped:Connect(function() -- put everything in this loop
	if menu:GetVal("Misc", "Miscellaneous", "Remove Kill Bricks") == true then
		game.Terrain.Water:Destroy()a
	else
		wait()
	end
]]
	...
	menu.connections.renderstepped2 = game.RunService.RenderStepped:Connect(function() -- put everything in this loop
		menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
			if name == "Button" or "Button Double Click" and tab == "Placeholder" then
				print("Button Clicked")
			end
		end)
	
		if menu:GetVal("Placeholder", "Tab Holder 1", "Toggle") == true then
			wait(1)
			print("true")
		else
			wait()
		...
		if menu:GetVal("Placeholder", "Tab Holder 1", "Slider") then
			Camera.FieldOfView = (menu:GetVal("Placeholder", "Tab Holder 1", "Slider"))
		end

		
		if menu:GetVal("Placeholder", "Tab Holder 1", "TextBox") and menu:GetVal("Placeholder", "Tab Holder 1", "Toggle Text Print") then
			print(menu:GetVal("Placeholder", "Tab Holder 1", "TextBox"))
		end
	...
	menu.Initialize({
		{
			name = "Sakura Stand", -- add tab
			content = {
				{
					name = "Autofarm Section", -- add section
					autopos = "left",
					autofill = false,
					content = {
                        {
							type = TOGGLE, -- add toggle
							name = "Auto Farm Selected NPC",
							value = false,
						},
						{
							type = TOGGLE, -- add toggle
							name = "Auto Farm Block DMG",
							value = false,
						},
                        {
							type = DROPBOX,
							name = "NPCs",
							value = 1,
							values = { "None", "Dummy", "Bandit", "Jotaro Kujo", "Chariot Requiem","Reflecting Dummy","Thug","Attacking Dummy"},
						},
						...
						--	type = COMBOBOX,
						--	name = "Target",
						--	values = { { "Players", true }, { "Mobs", false } },
						--},
						--{
						--	type = TOGGLE, -- add toggle
						--	name = "Toggle Text Print",
						--	value = false,
						--},
						--{
						--	type = TEXTBOX,
				...
				{
					name = "Auto Farm Settings", -- add section
					autopos = "left",
					autofill = true,
					content = {
						{
                ...
                {
					name = "Auto Sell Section", -- add section
					autopos = "right",
					autofill = false,
					content = {
						{
						...
						},
                        {
                            type = COMBOBOX,
                            name = "Selected Items",
                            values = { { "Arrow", false }, { "Mysterious Camera", false }, { "Hamon Manual", false } , { "Rokakaka", false }, { "Stop Sign", false } , { "Stone Mask", false } , { "Haunted Sword", false }, { "Spin Manual", false } },
                        },
					},
				},
                {
					name = "Other Section", -- add section
					autopos = "right",
					autofill = true,
					content = {
                        {
		...
		},
		{
			...
			content = {
				{
					name = {"Player ESP", "Mob ESP","Other ESP"},
					autopos = "left",
					size = 229,
					subtabfill = true,
					[1] = {
						content = {
							...
							},
							{
								...
								extra = {
									...
									color = { {0, 195, 255, 0 }, { 0, 195, 255, 300 } },
								},
							},
							{
								type = TOGGLE,
						...
						content = {
							...
							},
							{
								...
								extra = {
									type = DOUBLE_COLORPICKER,
									name = { "Box Fill", "Box ESP" },
									color = { { 255, 255, 255, 0 }, { 255, 255, 255, 300 } },
								},
							},
							{
								type = TOGGLE,
					...
					[3] = {
						content = {
							...
							
							{
								...
								extra = {
									type = COLORPICKER,
									name = "Fallen Soul ESP",
									color = { 100, 75, 255, 255 },
								},
							},
							{
								type = SLIDER,
								name = "Fallen Soul Distance",
								value = 7500,
								minvalue = 50,
								maxvalue = 15000,
								stradd = "m",
				...
				{
					name = {"Player Settings", "Mob Settings"},
					autopos = "left",
					autofill = true,
					[1] = {
						content = {
							{
								type = COMBOBOX,
								name = "Checks",
								values = { { "Alive", true }, { "Distance", false }, { "Local Player", true } },
							},
							{
								type = SLIDER,
								name = "Max Distance",
					...
					[1] = {
						content = {
							...
							},
							{
								...
								name = "No Blur",
								value = true,
								tooltip = "Removes Blur from screen (like from water and spit)",
							},
							{
								type = TOGGLE,
								name = "No Fog",
			...
			content = {
				...
				},
                {
					...
					content = {
                        ...
                        {
							type = BUTTON,
							name = "Hikarishi (Anubis)",
							doubleclick = true,
						},
                        {
							type = BUTTON,
							name = "Luna (Sell Items)",
							doubleclick = true,
						},
                        {
							type = BUTTON,
			...
			content = {
				...
				},
				{
					...
					content = {
						...
						{
							type = BUTTON,
							name = "Set Clipboard Job ID",
						},
						{
							type = BUTTON,
							name = "Set Clipboard Place ID",
						},
						{
							type = BUTTON,
							name = "Set Clipboard Position",
						},
						{
							type = BUTTON,
							name = "Unload Cheat",
	...
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Server Hop" then
			--Teleport()
			CreateNotification("Not working rn")
		end
	end)
    menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Spin Arcade" then
	...
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Set Clipboard Position" and tab == "Settings" then
			local char = LOCAL_PLAYER.Character
			local pos = char.HumanoidRootPart
			pos = pos.Position
			if not pos then
				return CreateNotification("Failed to Copy Position")
			end
			local roundedPos = math.round(pos.X) .. ", " .. math.round(pos.Y) .. ", " .. math.round(pos.Z)
			setclipboard(roundedPos)
			CreateNotification("Set Clipboard Position " .. roundedPos)
		end
	end)
	...
	local Notification = loadstring(
		game:HttpGet("https://raw.githubusercontent.com/xzqecc/Script/main/notifbackup", true)
	)()

	game.Players.PlayerAdded:Connect(function(player)
		for i, v in pairs(game.Players:GetChildren()) do
			if v == player then
				if v:GetRankInGroup(5212858) == 10 then
					Notification.new("warning", "Tester Detected", "" .. v.Name .. " Rank: Tester")
				elseif v:GetRankInGroup(5212858) == 250 then
					Notification.new("warning", "Moderator Detected", "" .. v.Name .. ", Rank: Moderator")
				elseif v:GetRankInGroup(5212858) == 254 or v:GetRankInGroup(5212858) == 253 or v:GetRankInGroup(5212858) == 252 or v:GetRankInGroup(5212858) == 251 then
					Notification.new("warning", "Admin Detected", "" .. v.Name .. ", Rank: Admin")
				elseif v:GetRankInGroup(5212858) == 255 then
					Notification.new("warning", "Owner Detected", "" .. v.Name .. ", Rank: Owner")
				end
			end
		end
	end)
	...
	game.Players.PlayerRemoving:Connect(function(player)
		for i, v in pairs(game.Players:GetChildren()) do
			if v == player then
				if v:GetRankInGroup(5212858) == 10 then
					Notification.new("warning", "Tester Left", "" .. v.Name .. " Rank: Tester")
				elseif v:GetRankInGroup(5212858) == 250 then
					Notification.new("warning", "Moderator Left", "" .. v.Name .. ", Rank: Moderator")
				elseif v:GetRankInGroup(5212858) == 254 or v:GetRankInGroup(5212858) == 253 or v:GetRankInGroup(5212858) == 252 or v:GetRankInGroup(5212858) == 251 then
					Notification.new("warning", "Admin Left", "" .. v.Name .. ", Rank: Admin")
				elseif v:GetRankInGroup(5212858) == 255 then
					Notification.new("warning", "Owner Left", "" .. v.Name .. ", Rank: Owner")
				end
			end
		end
	end)
    ...
    game:GetService("RunService").Stepped:Connect(function()
        ...
        if menu:GetVal("Sakura Stand","Autofarm Section","Item Aura") == true then
            for _, v in pairs(game:GetService("Workspace").Item:GetDescendants()) do
                if v:IsA("ClickDetector") then
                    fireclickdetector(v)
                end
            end
        end
        if menu:GetVal("Sakura Stand","Autofarm Section","Auto Farm Souls") == true then
            for i,v in pairs(game:GetService("Workspace").Item2:GetChildren()) do
                if v:FindFirstChild("ProximityPrompt") and v.ProximityPrompt:FindFirstChild("Pick") then
                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.CFrame
                    fireproximityprompt(v.ProximityPrompt,1,true)
                end
            end
        end
        --{ "None", "Dummy", "Bandit", "Jotaro Kujo", "Chariot Requiem"},
        if menu:GetVal("Sakura Stand","Autofarm Section","NPCs") ~= 1 and menu:GetVal("Sakura Stand","Autofarm Section","Auto Farm Selected NPC") == true then
            if menu:GetVal("Sakura Stand","Autofarm Section","NPCs") == 2 then
                for i, v in pairs(game:GetService("Workspace").Living:GetChildren()) do
                    if v.Name == "Dummy" and v.Humanoid.Health ~= 0 then
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame
                            * CFrame.new(
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "x Offset"),
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "y Offset"),
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "z Offset")
                            )
                    end
                end
            elseif menu:GetVal("Sakura Stand","Autofarm Section","NPCs") == 3 then
                for i, v in pairs(game:GetService("Workspace").Living:GetChildren()) do
                    if string.find(v.Name, "Bandit") and v.Humanoid.Health ~= 0 then
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame
                            * CFrame.new(
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "x Offset"),
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "y Offset"),
            ...
            elseif menu:GetVal("Sakura Stand","Autofarm Section","NPCs") == 4 then
                for i, v in pairs(game:GetService("Workspace").Living:GetChildre1n()) do
                    if v.Name == "Jotaro Kujo" and v.Humanoid.Health ~= 0 then
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame
                            * CFrame.new(
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "x Offset"),
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "y Offset"),
            ...
            elseif menu:GetVal("Sakura Stand","Autofarm Section","NPCs") == 5 then
                for i, v in pairs(game:GetService("Workspace").Living:GetChildren()) do
                    if v.Name == "Chariot Requiem" and v.Humanoid.Health ~= 0 then
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame
                            * CFrame.new(
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "x Offset"),
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "y Offset"),
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "z Offset")
                            )end end elseif menu:GetVal("Sakura Stand","Autofarm Section","NPCs") == 6 then for i, v in pairs(game:GetService("Workspace").Living:GetChildren()) do if v.Name == "Reflecting Dummy" and v.Humanoid.Health ~= 0 then game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame* CFrame.new( menu:GetVal("Sakura Stand", "Auto Farm Settings", "z Offset")
                            )
                    end
                end
            elseif menu:GetVal("Sakura Stand","Autofarm Section","NPCs") == 7 then
                for i, v in pairs(game:GetService("Workspace").Living:GetChildren()) do
                    if v.Name == "Thug" and v.Humanoid.Health ~= 0 then
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame
                            * CFrame.new(
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "x Offset"),
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "y Offset"),
            ...
            elseif menu:GetVal("Sakura Stand","Autofarm Section","NPCs") == 8 then
                for i, v in pairs(game:GetService("Workspace").Living:GetChildren()) do
                    if v.Name == "Attacking Dummy" and v.Humanoid.Health ~= 0 then
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame
                            * CFrame.new(
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "x Offset"),
                                menu:GetVal("Sakura Stand", "Auto Farm Settings", "y Offset"),
		...
		if menu:GetVal("Sakura Stand","Autofarm Section", "Auto Farm Block DMG") == true then
			for i, v in pairs(game:GetService("Workspace").Living:GetChildren()) do
				if v.Name == "Attacking Dummy" and v.Humanoid.Health ~= 0 and game.Players.LocalPlayer.Character.UsedValues.BlockBar.Value == 50 then
					game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.HumanoidRootPart.CFrame * CFrame.new(0,0,-3)
					keypress(0x46)
				elseif game.Players.LocalPlayer.Character.UsedValues.BlockBar.Value < 5 then
					game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-645, 129, 1947)
					keyrelease(0x46)
				end
			end
		end
        if
            menu:GetVal("Sakura Stand","Autofarm Section", "Auto Farm Selected NPC") == true
            and menu:GetVal("Sakura Stand","Autofarm Section","NPCs") ~= 1
        then
            if menu:GetVal("Sakura Stand", "Auto Farm Settings", "Auto M1") == true then
                game:GetService('VirtualInputManager'):SendMouseButtonEvent(0,0,0,true,game,0)
                wait()
                game:GetService('VirtualInputManager'):SendMouseButtonEvent(0,0,0,false,game,0)
            end
            if menu:GetVal("Sakura Stand", "Auto Farm Settings", "Move E") == true then
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "E", false, game)
            end
            if menu:GetVal("Sakura Stand", "Auto Farm Settings", "Move R") == true then
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "R", false, game)
            end
            if menu:GetVal("Sakura Stand", "Auto Farm Settings", "Move T") == true then
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "T", false, game)
            end
            if menu:GetVal("Sakura Stand", "Auto Farm Settings", "Move Y") == true then
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "Y", false, game)
            end
            if menu:GetVal("Sakura Stand", "Auto Farm Settings", "Move G") == true then
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "G", false, game)
            end
            if menu:GetVal("Sakura Stand", "Auto Farm Settings", "Move H") == true then
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "H", false, game)
            end
            if menu:GetVal("Sakura Stand", "Auto Farm Settings", "Move J") == true then
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "J", false, game)
            end
        end
        if menu:GetVal("Sakura Stand","Auto Sell Section", "Auto Sell") == true then
            local items = menu:GetVal("Sakura Stand","Auto Sell Section", "Selected Items")
            -- { "Arrow", false }, { "Mysterious Camera", false }, { "Hamon Manual", false } , { "Rokakaka", false }, { "Stop Sign", false } , 
            --{ "Stone Mask", false } , { "Haunted Sword", false }, { "Spin Manual", false } },
            if items[1] then
                game:GetService("ReplicatedStorage").GlobalUsedRemotes.SellItem:FireServer("Arrow")
            end
            if items[2] then
                game:GetService("ReplicatedStorage").GlobalUsedRemotes.SellItem:FireServer("Mysterious Camera")
            end
            if items[3] then
                game:GetService("ReplicatedStorage").GlobalUsedRemotes.SellItem:FireServer("Hamon Manual")
            end
            if items[4] then
                game:GetService("ReplicatedStorage").GlobalUsedRemotes.SellItem:FireServer("Rokakaka")
            end
            if items[5] then
                game:GetService("ReplicatedStorage").GlobalUsedRemotes.SellItem:FireServer("Stop Sign")
            end
            if items[6] then
                game:GetService("ReplicatedStorage").GlobalUsedRemotes.SellItem:FireServer("Stone Mask")
            end
            if items[7] then
                game:GetService("ReplicatedStorage").GlobalUsedRemotes.SellItem:FireServer("Haunted Sword")
            end
            if items[8] then
                game:GetService("ReplicatedStorage").GlobalUsedRemotes.SellItem:FireServer("Spin Manual")
            end
        end
    end)
    menu.connections.renderstepped2 = game.RunService.RenderStepped:Connect(function()
        if menu:GetVal("Sakura Stand","Autofarm Section","Auto Farm Boxes") == true then
            getgenv().noclip = true
            for i,v in pairs(game:GetService("Workspace").Item:GetDescendants()) do
                if string.find(v.Name, "Space") and v.Parent.Name == "Box" and v.Parent:FindFirstChild("Left") and not v.Parent.AC:FindFirstChild("Animator")  then
                    for x,z in pairs(game.Players.LocalPlayer:GetChildren()) do 
                        if  not string.find(z.Name,v.Name) then
                            
                            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.Parent.Left.CFrame + Vector3.new(5,0,10)
                        end
                    end
                end
            end
    ...
    menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Asakura (Arcade)" then
			LOCAL_PLAYER.Character.HumanoidRootPart.CFrame = CFrame.new(-194.126, -30.5598, 1464.36)
		end
	end)
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Hikarishi (Anubis)" then
			LOCAL_PLAYER.Character.HumanoidRootPart.CFrame = CFrame.new(-508.402, -31.7678, 1504.47)
		end
	end)
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Luna (Sell Items)" then
			LOCAL_PLAYER.Character.HumanoidRootPart.CFrame = CFrame.new(-619.713, -31.0723, 1921.97)
		end
	end)
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Umbra" then
			LOCAL_PLAYER.Character.HumanoidRootPart.CFrame = CFrame.new(-377.307, -30.0448, 1826.91)
		end
	end)
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Auddy (Shop)" then
			LOCAL_PLAYER.Character.HumanoidRootPart.CFrame = CFrame.new(-377.344, -30.0331, 1826.93)
		end
	end)
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "XZ_LF (Guild)" then
			LOCAL_PLAYER.Character.HumanoidRootPart.CFrame = CFrame.new(-695.705, -31.3541, 1651.71)
		end
	end)
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "SwordsMan(Skin Remover)" then
			LOCAL_PLAYER.Character.HumanoidRootPart.CFrame = CFrame.new(57.5007, -31.0821, 1576.69)
		end
	end)
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Midknight" then
			LOCAL_PLAYER.Character.HumanoidRootPart.CFrame = CFrame.new(-218.864, -31.252, 1718.32)
		end
	end)
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Mochi Quest" then
			LOCAL_PLAYER.Character.HumanoidRootPart.CFrame = CFrame.new(-81.3614, -115.366, 360.266)
		end
	end)
	local function unikeycheck(actionName, inputState, inputObject)
		if actionName == "BB UNI check" then
			if inputState == Enum.UserInputState.Begin then
				if
					menu:GetVal("Misc", "Movement", "Mouse Teleport")
					and inputObject.KeyCode == menu:GetVal("Misc", "Movement", "Mouse Teleport", KEYBIND)
				then
	...
	local function SpeedHack()
		local speed = menu:GetVal("Movement", "Movement Section", "Speed Factor")

		if
			menu:GetKey("Movement", "Movement Section", "Speed")
			and LOCAL_PLAYER.Character
			and LOCAL_PLAYER.Character.Humanoid
		then
			if menu:GetVal("Movement", "Movement Section", "Speed Method") == 1 then
				...
				if rootpart ~= nil then
					...
					if INPUT_SERVICE:IsKeyDown(Enum.KeyCode.W) then
						travel = travel + Vector3.new(looking.x, 0, looking.Z)
					end
	...
	local function Noclip()
		if menu:GetVal("Movement", "Movement Section", "Noclip") then
			cachedValues.Nocliptoggle = menu:GetKey("Movement", "Movement Section", "Noclip")
			if cachedValues.Nocliptoggle then
				getgenv().noclip = true
			else
				getgenv().noclip = false
			end
	...
	local function FlyHack()
		if menu:GetVal("Movement", "Movement Section", "Fly") then
			local rootpart = LOCAL_PLAYER.Character:FindFirstChild("HumanoidRootPart")

			if menu:GetVal("Movement", "Movement Section", "Fly Method") == 2 then
				getgenv().noclip = true
			end
			cachedValues.FlyToggle = menu:GetKey("Movement", "Movement Section", "Fly")
			if cachedValues.FlyToggle then
				local speed = menu:GetVal("Movement", "Movement Section", "Fly Speed")
				getgenv().noclip = true
				local travel = Vector3.new()
				...
				do
					...
					if INPUT_SERVICE:IsKeyDown(Enum.KeyCode.Space) then
						travel = travel+Vector3.new(0, 1, 0)
					end
					if INPUT_SERVICE:IsKeyDown(Enum.KeyCode.LeftShift) then
						travel = travel-Vector3.new(0, 1, 0)
					end
				...
				--cachedValues.Nocliptoggle = false
			end
		end
	end
	...
	local function updateplist()
		...
		for k, v in pairs(playerz) do
			...
			local vfirstlast = v:GetAttribute("FirstName") or v:GetAttribute("LastName")
			if vfirstlast ~= nil then
				firstlast = v:GetAttribute("FirstName").. " " .. v:GetAttribute("LastName")
			else
				firstlast = "nil"
			end
			local teamtext = {
			...
			if v == LOCAL_PLAYER then
				playername[1] = "Local Player"
				playername[2] = RGB(66, 135, 245)
				teamtext[1] = "Local Player"
				teamtext[2] = RGB(66, 135, 245)
			...
			if player.Character ~= nil then
				...
				if humanoid ~= nil then
					if humanoid.Health ~= nil then
						playerhealth = tostring(math.round(humanoid.Health))
							.. "/"
							.. tostring(math.round(humanoid.MaxHealth))
					else
	...
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Teleport" then
			...
			elseif selectedPlayer == LOCAL_PLAYER then
				...
				if character then
					local targetPos = character.HumanoidRootPart

					if targetPos then
						local targetRelPos = targetPos.Position
						if targetRelPos then
							local RP = LOCAL_PLAYER.Character.HumanoidRootPart
							RP.CFrame = Tween(cframenew(targetRelPos))
							CreateNotification('Teleported to "' .. selectedPlayer.Name .. '" using part: ')
						else
	...
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Spectate" then
			if selectedPlayer == nil then
				CreateNotification("No player selected")
			elseif selectedPlayer == LOCAL_PLAYER then
				CreateNotification("You can't Spectate yourself silly ")
			else
				local character = selectedPlayer.Character
				...
				if character then
					...
					if getgenv().spectate == true then
						spoof(game:GetService("Workspace").Camera, "CameraSubject", character.Humanoid)

						CreateNotification("Spectating " .. selectedPlayer.Name .. " Using Part: Humanoid")
					end
				else
					CreateNotification("Failed Spectate, couldn't find player character")
				end
			end
		end
	end)
	...
	menu.connections.button_pressed_uni = ButtonPressed:connect(function(tab, gb, name)
		if name == "Unspectate" then
			...
			elseif selectedPlayer == LOCAL_PLAYER then
				...
				if character then
					unspoof(game:GetService("Workspace").Camera, "CameraSubject", character.Humanoid)

					CreateNotification("Unspectated " .. selectedPlayer.Name)
				else
					CreateNotification("Failed Spectate, couldn't find player character")
				end
			end
	...
	menu.connections.renderstepped2 = game.RunService.RenderStepped:Connect(function()
		...
		local organizedBoxes = {}
		for i,v in ipairs(game:GetService("Workspace").Item:GetChildren()) do
			if v:IsA("Model") and v:FindFirstChild("Left") then
				if
					menu:GetVal("Visuals", "Other ESP", "Box Distance")
					and LOCAL_PLAYER:DistanceFromCharacter(v.Left.Position) / 5
						> menu:GetVal("Visuals", "Other ESP", "Box Distance")
				then
					continue
				end
				table.insert(organizedBoxes, v)
			end
		end
		local organizedSouls = {}
		for i,v in ipairs(game:GetService("Workspace").Item2:GetChildren()) do
			if v:IsA("Part") and v:FindFirstChild("ProximityPrompt") then
				if
					menu:GetVal("Visuals", "Other ESP", "Fallen Soul Distance")
					and LOCAL_PLAYER:DistanceFromCharacter(v.Position) / 5
						> menu:GetVal("Visuals", "Other ESP", "Fallen Soul Distance")
				then
					continue
				end
				table.insert(organizedSouls, v)
			end
		end
		local organizedNPCs = {}
		for i,v in ipairs(game:GetService("Workspace").Map.NPCs:GetChildren()) do
			if v:IsA("Model") and v:FindFirstChild("HumanoidRootPart") then
				if
					menu:GetVal("Visuals", "Other ESP", "NPC Distance")
					and LOCAL_PLAYER:DistanceFromCharacter(v.HumanoidRootPart.Position) / 5
						> menu:GetVal("Visuals", "Other ESP", "NPC Distance")
				then
					continue
				end
				table.insert(organizedNPCs, v)
			end
		end
		local organizedPlayers = {}
		for i, v in ipairs(Players:GetPlayers()) do
			if v.Character ~= nil and v.Character:FindFirstChild("HumanoidRootPart") then
				local checks = menu:GetVal("Visuals", "Player Settings", "Checks")
				...
				if
					checks[2]
					and LOCAL_PLAYER:DistanceFromCharacter(v.Character.HumanoidRootPart.Position) / 5
						> menu:GetVal("Visuals", "Player Settings", "Max Distance")
				then
					continue
				end

                if checks[3] and v.Name == LOCAL_PLAYER.Name then
                    continue
                end
		...
		for i, v in ipairs(game:GetService("Workspace").Living:GetChildren()) do
			if v:FindFirstChild("HumanoidRootPart") and not game:FindService("Players"):FindFirstChild(v.Name) then
				if menu:GetVal("Visuals", "Mob Settings", "Checks")[1] and LOCAL_PLAYER:DistanceFromCharacter(v.HumanoidRootPart.Position) / 5 > menu:GetVal("Visuals", "Mob Settings", "Max Distance") then
					continue
				end
			table.insert(organizedMobs, v)
			end
		end
		...
		local priority_c = menu:GetVal("Visuals", "Player Settings", "Highlight Priority", COLOR, true)
		local priority_t = menu:GetVal("Visuals", "Player Settings", "Highlight Priority", COLOR)[4] / 255

		local friend_c = menu:GetVal("Visuals", "Player Settings", "Highlight Friends", COLOR, true)
		local friend_t = menu:GetVal("Visuals", "Player Settings", "Highlight Friends", COLOR)[4] / 255

		local target_c = menu:GetVal("Visuals", "Player Settings", "Highlight Target", COLOR, true)
		local target_t = menu:GetVal("Visuals", "Player Settings", "Highlight Target", COLOR)[4] / 255

		local textcase = menu:GetVal("Visuals", "Player Settings", "Text Case")
		local textlength = menu:GetVal("Visuals", "Player Settings", "Max Text Length")
		...
		for i,v in ipairs(organizedBoxes) do
			pcall(function()
				local rootpart = v.Left.Position
				local cam = Camera.CFrame
				local torso = v.Left.CFrame
				local head = v.Left.CFrame
				local top, top_isrendered = workspace.CurrentCamera:WorldToViewportPoint(
					head.Position + (torso.UpVector * 1) + cam.UpVector
				)
				local bottom, bottom_isrendered = workspace.CurrentCamera:WorldToViewportPoint(
					torso.Position - (torso.UpVector * 2.5) - cam.UpVector
				)
				...
				if top_isrendered or bottom_isrendered then
					...
					if menu:GetVal("Visuals", "Other ESP", "Boxes") then
						local dist_pos = Vector2.new(math.floor(boxtop.x + boxsize.w * 0.5), boxtop.y + boxsize.h)
						allesp.Box[i].Text = formattext("[Box]".. "[" ..math.ceil(LOCAL_PLAYER:DistanceFromCharacter(rootpart) / 5) .. "m]"
							
						)
						allesp.Box[i].Position = Vector2.new(
							math.floor(boxtop.x + boxsize.w * 0.5),
							math.floor(boxtop.y - 0)
						)
						allesp.Box[i].Visible = true
						allesp.Box[i].Color = menu:GetVal(
							"Visuals",
							"Other ESP",
							"Boxes",
							COLOR,
						...
						allesp.Box[i].Transparency = menu:GetVal(
							"Visuals",
							"Other ESP",
							"Boxes",
							COLOR
		...
		for i,v in ipairs(organizedNPCs) do
			pcall(function()
				local rootpart = v.HumanoidRootPart.Position
				local cam = Camera.CFrame
				local torso = v.HumanoidRootPart.CFrame
				local head = v.HumanoidRootPart.CFrame
				local top, top_isrendered = workspace.CurrentCamera:WorldToViewportPoint(
					head.Position + (torso.UpVector * 1) + cam.UpVector
				)
				local bottom, bottom_isrendered = workspace.CurrentCamera:WorldToViewportPoint(
				...
				if top_isrendered or bottom_isrendered then
					local boxtop = Vector2.new(math.floor(top.x * 0.5 + bottom.x * 0.5 - sizeX * 0.5), math.floor(math.min(top.y, bottom.y)))
					local boxsize = { w = sizeX, h = sizeY }
					if menu:GetVal("Visuals", "Other ESP", "NPC") then
						local dist_pos = Vector2.new(math.floor(boxtop.x + boxsize.w * 0.5), boxtop.y + boxsize.h)
						allesp.Box[i].Text = formattext("[".. v.Name .."]".. "[" ..math.ceil(LOCAL_PLAYER:DistanceFromCharacter(rootpart) / 5) .. "m]"
        ...
        for i,v in ipairs(organizedSouls) do
			pcall(function()
				local rootpart = v.Position
				local cam = Camera.CFrame
				local torso = v.CFrame
				local head = v.CFrame
				local top, top_isrendered = workspace.CurrentCamera:WorldToViewportPoint(
					head.Position + (torso.UpVector * 1) + cam.UpVector
				)
				...
				if top_isrendered or bottom_isrendered then
					local boxtop = Vector2.new(math.floor(top.x * 0.5 + bottom.x * 0.5 - sizeX * 0.5), math.floor(math.min(top.y, bottom.y)))
					local boxsize = { w = sizeX, h = sizeY }
					if menu:GetVal("Visuals", "Other ESP", "Fallen Soul") then
						local dist_pos = Vector2.new(math.floor(boxtop.x + boxsize.w * 0.5), boxtop.y + boxsize.h)
						allesp.Box[i].Text = formattext("[Fallen Soul]".. "[" ..math.ceil(LOCAL_PLAYER:DistanceFromCharacter(rootpart) / 5) .. "m]"
		...
		for i, v in ipairs(organizedMobs) do
			pcall(function()
				local humanoid = v:FindFirstChild("Humanoid")
				local rootpart = v.HumanoidRootPart.Position
				local cam = Camera.CFrame
				local torso = v.Torso.CFrame
				...
				if top_isrendered or bottom_isrendered then
					local boxtop = Vector2.new(math.floor(top.x * 0.5 + bottom.x * 0.5 - sizeX * 0.5), math.floor(math.min(top.y, bottom.y)))
					local boxsize = { w = sizeX, h = sizeY }
					if not game:FindService("Players"):FindFirstChild(v.Name) then
						if menu:GetVal("Visuals", "Mob ESP", "Boxes") then
							allesp.mobouterbox[i].Position = Vector2.new(boxtop.x - 1, boxtop.y - 1)
							allesp.mobouterbox[i].Size = Vector2.new(boxsize.w + 2, boxsize.h + 2)
							allesp.mobouterbox[i].Visible = true

							allesp.mobinnerbox[i].Position = Vector2.new(boxtop.x + 1, boxtop.y + 1)
							allesp.mobinnerbox[i].Size = Vector2.new(boxsize.w - 2, boxsize.h - 2)
							allesp.mobinnerbox[i].Visible = true

							allesp.mobbox[i].Position = Vector2.new(boxtop.x, boxtop.y)
							allesp.mobbox[i].Size = Vector2.new(boxsize.w, boxsize.h)
							allesp.mobbox[i].Visible = true

							if menu:GetVal("Visuals", "Mob ESP", "Boxes", COLOR1)[4] / 255 > 0 then
								allesp.mobfilledbox[i].Position = Vector2.new(boxtop.x + 2, boxtop.y + 2)
								allesp.mobfilledbox[i].Size = Vector2.new(boxsize.w - 4, boxsize.h - 4)
								allesp.mobfilledbox[i].Visible = true
							end
						end
						if humanoid then
							local health = math.ceil(humanoid.Health)
							local maxhealth = math.ceil(humanoid.MaxHealth)
							if menu:GetVal("Visuals", "Mob ESP", "Health Bar") then
								allesp.mobhealthouter[i].Position = Vector2.new(boxtop.x - 6, boxtop.y - 1)
								allesp.mobhealthouter[i].Size = Vector2.new(4, boxsize.h + 2)
								allesp.mobhealthouter[i].Visible = true

								local ySizeBar = -math.floor(boxsize.h * health / maxhealth)

								allesp.mobhealthinner[i].Position = Vector2.new(boxtop.x - 5, boxtop.y + boxsize.h)
								allesp.mobhealthinner[i].Size = Vector2.new(2, ySizeBar)
								allesp.mobhealthinner[i].Visible = true
								allesp.mobhealthinner[i].Color = ColorRange(health, {
									[1] = {
										start = 0,
										color = menu:GetVal("Visuals", "Mob ESP", "Health Bar", COLOR1, true),
									},
									[2] = {
										start = 100,
										color = menu:GetVal("Visuals", "Mob ESP", "Health Bar", COLOR2, true),
									},
								})
								local r = (math.ceil(health) / math.ceil(maxhealth)) * 100
								if menu:GetVal("Visuals", "Mob ESP", "Health Number") then
									allesp.mobhptext[i].Text = tostring(health .. "/" .. maxhealth)
									local textsize = allesp.mobhptext[i].TextBounds
									allesp.mobhptext[i].Position = Vector2.new(
										boxtop.x - 7 - textsize.x,
										boxtop.y + clamp(boxsize.h + ySizeBar - 8, -4, boxsize.h - 10)
									)
									allesp.mobhptext[i].Visible = true
									allesp.mobhptext[i].Color = menu:GetVal(
										"Visuals",
										"Mob ESP",
										"Health Number",
										COLOR,
									...
									allesp.mobhptext[i].Transparency = menu:GetVal(
										"Visuals",
										"Mob ESP",
										"Health Number",
										COLOR
							...
							elseif menu:GetVal("Visuals", "Mob ESP", "Health Number") then
								allesp.mobhptext[i].Text = tostring(health .. "/" .. maxhealth)
								local textsize = allesp.mobhptext[i].TextBounds
								allesp.mobhptext[i].Position = Vector2.new(boxtop.x - 3 - textsize.x, boxtop.y - 4)
								allesp.mobhptext[i].Visible = true

							end
						end
						if menu:GetVal("Visuals", "Mob ESP", "Name") then
							allesp.mobname[i].Text = formattext(
								string.gsub(v.Name, "%d+", ""):gsub("[^%w%s]+", "")
							)
							allesp.mobname[i].Position = Vector2.new(
								math.floor(boxtop.x + boxsize.w * 0.5),
								math.floor(boxtop.y - 15)
							)
							allesp.mobname[i].Visible = true
						end
						if menu:GetVal("Visuals", "Mob ESP", "Distance") then
							local dist_pos = Vector2.new(math.floor(boxtop.x + boxsize.w * 0.5), boxtop.y + boxsize.h)
							allesp.mobdistance[i].Text = tostring(
								math.ceil(LOCAL_PLAYER:DistanceFromCharacter(rootpart) / 5)
							) .. "m"
							allesp.mobdistance[i].Position = dist_pos
							allesp.mobdistance[i].Visible = true
						end
					end
				end
			end)
				...
				if top_isrendered or bottom_isrendered then
					...
					if menu:GetVal("Visuals", "Player ESP", "Head Dot") then
						...
						if head then
							...
							local headdotpos = workspace.CurrentCamera:WorldToViewportPoint(
								Vector3.new(headpos.x, headpos.y + 0.1, headpos.z)
							)
							local headdotpos_b = workspace.CurrentCamera:WorldToViewportPoint(
								Vector3.new(headpos.x, headpos.y - 0.2, headpos.z)
							)
					...
					if menu:GetVal("Visuals", "Player ESP", "Boxes") then
						...
						if menu:GetVal("Visuals", "Player ESP", "Boxes", COLOR1)[4] / 255 > 0 then
					...
					if humanoid then
						...
						if menu:GetVal("Visuals", "Player ESP", "Health Bar") then
							...
							local r = (math.ceil(health) / math.ceil(maxhealth)) * 100
							if
								menu:GetVal("Visuals", "Player ESP", "Health Number")
								and menu:GetVal("Visuals", "Player Settings", "Max HP Visibility Cap") >= math.ceil(r)
							then
								allesp.hptext[i].Text = tostring(health .. "/" .. maxhealth)
								...
								allesp.hptext[i].Color = menu:GetVal(
									"Visuals",
									"Player ESP",
									"Health Number",
									COLOR,
								...
								allesp.hptext[i].Transparency = menu:GetVal(
									"Visuals",
									"Player ESP",
									"Health Number",
									COLOR
						...
						elseif
							menu:GetVal("Visuals", "Player ESP", "Health Number")
							and menu:GetVal("Visuals", "Player Settings", "Max HP Visibility Cap")
								>= math.ceil((health / maxhealth) * 100)
						then
							allesp.hptext[i].Text = tostring(health .. "/" .. maxhealth)
							local textsize = allesp.hptext[i].TextBounds
							...
							allesp.hptext[i].Transparency = menu:GetVal("Visuals", "Player ESP", "Health Number", COLOR)[4]
								/ 255
						end
					end
					if menu:GetVal("Visuals", "Player ESP", "Name") then
						allesp.name[i].Text = formattext(v.Name)
						allesp.name[i].Position = Vector2.new(math.floor(boxtop.x + boxsize.w * 0.5), math.floor(boxtop.y - 15))
						allesp.name[i].Visible = true
					end


					if menu:GetVal("Visuals", "Player ESP", "Distance") then
						local dist_pos = Vector2.new(
							math.floor(boxtop.x + boxsize.w * 0.5),
							boxtop.y + boxsize.h + y_spot
						)
					...
					if
						menu:GetVal("Visuals", "Player Settings", "Highlight Priority")
					...
					then
						...
						allesp.filledbox[i].Transparency = menu:GetVal("Visuals", "Player ESP", "Boxes", COLOR1)[4] / 255
						allesp.filledbox[i].Color = target_c

						allesp.name[i].Color = priority_c
						allesp.name[i].Transparency = priority_t

						allesp.talents[i].Color = priority_c
						allesp.talents[i].Transparency = priority_t
					...
					elseif
						menu:GetVal("Visuals", "Player Settings", "Highlight Friends")
						and table.find(menu.friends, v.Name)
					then
						...
						allesp.talents[i].Color = friend_c
						allesp.talents[i].Transparency = friend_t
					...
					elseif menu:GetVal("Visuals", "Player Settings", "Highlight Target") and v == menu.aimbot_target then
						...
						allesp.talents[i].Color = target_c
						allesp.talents[i].Transparency = target_t
						...
						local boxt = menu:GetVal("Visuals", "Player ESP", "Boxes", COLOR2)[4] / 255
						allesp.box[i].Color = menu:GetVal("Visuals", "Player ESP", "Boxes", COLOR2, true)
						...
						allesp.mobfilledbox[i].Transparency = menu:GetVal("Visuals", "Mob ESP", "Boxes", COLOR1)[4]
							/ 255
						allesp.mobfilledbox[i].Color = menu:GetVal("Visuals", "Mob ESP", "Boxes", COLOR1, true)
						...
						allesp.Box[i].Color = menu:GetVal("Visuals", "Other ESP", "Boxes", COLOR, true)
						allesp.name[i].Transparency = menu:GetVal("Visuals", "Other ESP", "Boxes", COLOR)[4] / 255

						allesp.talents[i].Color = menu:GetVal("Visuals", "Player ESP", "Talents", COLOR, true)
						allesp.talents[i].Transparency = menu:GetVal("Visuals", "Player ESP", "Talents", COLOR)[4] / 255
...
do
	...
	wm.pos = Vector2.new(40, 35)
	wm.text = {}
	local fulltext = getgenv().menuname .. wm.textString
...
CreateThread(function()
	...
	for i = 1, 20 do
		...
		loadingthing.Position = loadingthing.Position - Vector2.new(x / 10, 0)
		wait()
	end
